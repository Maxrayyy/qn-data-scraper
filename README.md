...
5. 如出现滑块/无痕验证码，工具会暂停并提示"遇到验证码，请手动处理"——在浏览器窗口里手动滑一下，验证通过后工具自动继续；如出现**手机短信验证身份**，工具会自动点击"点击获取验证码"，请在浏览器窗口输入手机收到的 6 位数字并点"确定"；
6. 完成后弹出提示，Excel 保存在你选择的文件夹里，文件名 `千牛商品发现数据.xlsx`（若同名文件已存在，自动加时间戳，绝不覆盖旧文件）。

**关于登录状态（持久化会话）**：工具使用独立的内置浏览器档案（位于 `%APPDATA%\qn-data-scraper\browser-profile`），登录状态会被记住——首次运行时正常登录，之后的运行会**自动跳过登录**直接执行抓取。工具会以接近人工的节奏操作（随机停顿、模拟打字速度），并自动勾选"记住密码"，以降低触发平台风控的概率。

> ⚠️ **风控提醒**：任何自动化操作淘宝/千牛都可能有触发平台安全机制的风险。请**低频使用**（例如每天 1-2 次）；若收到平台的"异常访问行为"安全提示，请**立即停用工具**，用普通浏览器手动登录确认账号状态，待限制解除后再谨慎使用。
...
| 收到平台"异常访问行为"安全提示 | 自动化访问触发风控 | **立即停用工具**，普通浏览器手动登录确认账号，限制解除后低频使用 |
| 日志提示"登录失败：密码错误" | 账号密码有误 | 检查账号格式（子账号需 `主账号:子账号`） |
| 日志提示"启动持久化浏览器会话失败" | 已有工具实例在运行/上次未完全退出 | 关闭其他实例后重试；必要时重启电脑 |
...
- 浏览器会话档案（登录状态）保存在本机 `%APPDATA%\qn-data-scraper\browser-profile`，不包含配置文件中的密码。
...
- **持久化浏览器会话**：`app/worker.py` 用 `launch_persistent_context` 启动内置浏览器，会话档案在 `browser_profile_dir()`（Windows 为 `%APPDATA%\qn-data-scraper\browser-profile`）。登录状态跨运行保留；`app/steps.py` 的 `run_builtin_flow` 在检测到已登录时自动跳过登录步骤。首次使用需正常登录一次，之后低频使用基本免登录；
- **人工节奏模拟**：`app/steps.py` 中 `_human_click`（悬停+随机停顿+点击）、`_human_type`（逐字符随机间隔输入）、`_run_step` 与 `_step_collect` 的随机停顿。由 `StepsContext.human_pace` 控制（默认开启，测试中关闭）；
- **记住密码**：`_check_remember_password` 登录时尽力勾选"记住密码"，让平台信任本设备、减少短信验证；
- 注意：这些措施**降低**风险但**无法消除**——平台风控是此类工具的固有风险，向使用者如实说明（见用户章节的风控提醒）。
